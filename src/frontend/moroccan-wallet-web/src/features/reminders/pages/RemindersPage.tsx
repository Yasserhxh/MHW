...
export default function RemindersPage() {
  ...
  const grouped = useMemo(
    () => ({
      upcoming: (data ?? []).filter((item) => item.status === 'upcoming' || item.status === 'snoozed' || item.status === 'today'),
            ...
            <div className="space-y-3">
              {grouped[group].map((reminder) => (
                <div key={reminder.id} className="rounded-2xl border border-slate-200 p-4">
                  <Link className="text-sm font-semibold text-slate-900" to={`/reminders/${reminder.id}`}>
                    {reminder.title}
                  </Link>
                  <div className="mt-1 text-sm text-slate-500">
                    {reminder.dueDate} · {reminder.recurrence}
                  </div>
                  <div className="mt-3 flex gap-2">
                    {group !== 'completed' ? (
                      <Button size="sm" variant="outline" onClick={() => completeReminder.mutate(reminder.id)}>
                        Mark done
                      </Button>
                    ) : null}
                    {group === 'upcoming' || group === 'overdue' ? (
                      <Button size="sm" variant="ghost" onClick={() => snoozeReminder.mutate(reminder.id)}>
                        Snooze
                      </Button>
                    ) : null}
                  </div>
      ...
      <Drawer
        ...
        footer={
          <>
            ...
            <Button form="reminder-form" type="submit" loading={saveReminder.isPending}>
              Save
            </Button>
          </>
        }
      >
        <form
          ...
          onSubmit={handleSubmit(async (values) => {
            await saveReminder.mutateAsync({ payload: values });
            setOpen(false);
          })}
        >
          <Input label="Title" {...register('title')} />
          <div className="grid gap-4 sm:grid-cols-2">
            <Input label="Amount (optional)" type="number" {...register('amount', { valueAsNumber: true })} />
            <Input label="Due date" type="date" {...register('dueDate')} />
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <Input label="Category" {...register('category')} />
            <Input label="Recurrence" {...register('recurrence')} />
          </div>
