...
import { useNotificationActions, useNotifications, useUnreadNotificationCount } from '../hooks/useNotifications';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
        {unreadCount.isLoading ? 'Checking unread count...' : `${unreadCount.data ?? unread.length} unread notification(s)`}
      </div>
      {isEmpty ? (
        <div className="rounded-[1.75rem] border border-dashed border-slate-200 bg-white p-8 text-sm text-slate-500 shadow-sm">
          You are all caught up. New notifications will appear here as reminders and household activity come in.
        </div>
      ) : null}
      {!isEmpty &&
                      ...
                      {!item.isRead ? (
                        <div className="mt-2">
                          <Button
                            size="sm"
                            variant="ghost"
                            loading={actions.markRead.isPending && actions.markRead.variables === item.id}
